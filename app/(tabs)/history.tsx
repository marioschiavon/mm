...
import { History, Calendar, MapPin, Gauge, DollarSign, TrendingUp, Trash2 } from 'lucide-react-native';
import { useApp } from '@/contexts/AppContext';
import { LoadingScreen } from '@/components/LoadingScreen';

export default function HistoryScreen() {
  const { vehicle, refuels, getStationById, deleteRefuel, loading } = useApp();
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        {vehicle && (
          <Text style={styles.headerSubtitle}>{vehicle.name} • {vehicle.plate}</Text>
        )}
      </View>
      ...
      {sortedRefuels.length === 0 ? (
        ...
        <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
          ...
          {sortedRefuels.map((refuel, index) => {
            const station = getStationById(refuel.stationId);
            const isFirst = index === 0;
            const pricePerLiter = refuel.totalValue / refuel.liters;
